...
class BackupManager(private val context: Context, private val ioDispatcher: CoroutineDispatcher = Dispatchers.IO) {
    ...
    suspend fun createBackup(
        ...
        includeServerSettings: Boolean = false
        ...
        return@withContext try {
            ...
            val appSettings = if (includeServerSettings) {
                Logger.d(TAG, "   includeServerSettings=true, serverUrl=${prefs.getString(Constants.KEY_SERVER_URL, null)?.take(20).orEmpty()}")
                AppSettings(
                    // Server connection
                    ...
                    connectionTimeoutSeconds = prefs.getInt(Constants.KEY_CONNECTION_TIMEOUT_SECONDS, -1).takeIf { it >= 0 },
                    maxParallelConnections = prefs.getInt(Constants.KEY_MAX_PARALLEL_CONNECTIONS, -1).takeIf { it >= 0 },
                    // Sync behaviour
                    offlineMode = prefs.getBoolean(Constants.KEY_OFFLINE_MODE, false).takeIf { prefs.contains(Constants.KEY_OFFLINE_MODE) },
                    autoSync = prefs.getBoolean(Constants.KEY_AUTO_SYNC, false).takeIf { prefs.contains(Constants.KEY_AUTO_SYNC) },
                    wifiOnlySync = prefs.getBoolean(Constants.KEY_WIFI_ONLY_SYNC, false).takeIf { prefs.contains(Constants.KEY_WIFI_ONLY_SYNC) },
                    markdownExport = prefs.getBoolean(Constants.KEY_MARKDOWN_EXPORT, false).takeIf { prefs.contains(Constants.KEY_MARKDOWN_EXPORT) },
                    markdownAutoImport = prefs.getBoolean(Constants.KEY_MARKDOWN_AUTO_IMPORT, false).takeIf { prefs.contains(Constants.KEY_MARKDOWN_AUTO_IMPORT) },
                    alwaysCheckServer = prefs.getBoolean(Constants.KEY_ALWAYS_CHECK_SERVER, true).takeIf { prefs.contains(Constants.KEY_ALWAYS_CHECK_SERVER) },
                    alwaysDeleteFromServer = prefs.getBoolean(Constants.KEY_ALWAYS_DELETE_FROM_SERVER, false).takeIf { prefs.contains(Constants.KEY_ALWAYS_DELETE_FROM_SERVER) },
                    // Sync triggers
                    syncTriggerOnSave = prefs.getBoolean(Constants.KEY_SYNC_TRIGGER_ON_SAVE, true).takeIf { prefs.contains(Constants.KEY_SYNC_TRIGGER_ON_SAVE) },
                    syncTriggerOnResume = prefs.getBoolean(Constants.KEY_SYNC_TRIGGER_ON_RESUME, true).takeIf { prefs.contains(Constants.KEY_SYNC_TRIGGER_ON_RESUME) },
                    syncTriggerWifiConnect = prefs.getBoolean(Constants.KEY_SYNC_TRIGGER_WIFI_CONNECT, true).takeIf { prefs.contains(Constants.KEY_SYNC_TRIGGER_WIFI_CONNECT) },
                    syncTriggerPeriodic = prefs.getBoolean(Constants.KEY_SYNC_TRIGGER_PERIODIC, false).takeIf { prefs.contains(Constants.KEY_SYNC_TRIGGER_PERIODIC) },
                    syncTriggerBoot = prefs.getBoolean(Constants.KEY_SYNC_TRIGGER_BOOT, false).takeIf { prefs.contains(Constants.KEY_SYNC_TRIGGER_BOOT) },
                    syncIntervalMinutes = prefs.getLong(Constants.PREF_SYNC_INTERVAL_MINUTES, -1L).takeIf { it >= 0 },
                    // Display
                    ...
                    // Notes behaviour
                    autosaveEnabled = prefs.getBoolean(Constants.KEY_AUTOSAVE_ENABLED, true).takeIf { prefs.contains(Constants.KEY_AUTOSAVE_ENABLED) },
     ...
     */
    suspend fun backupContainsAppSettings(uri: Uri, password: String? = null): Boolean =
        withContext(ioDispatcher) {
            return@withContext try {
